...
// Create a VPC named "scaleup_sit_vpc"
resource "aws_vpc" "scaleup_sit_vpc" {
    ...
    // We are tagging the VPC with the name "scaleup_sit_vpc"
    tags = {
        Name = "scaleup_sit_vpc"
    }
}

// Create an internet gateway named "scaleup_sit_igw"
// and attach it to the "scaleup_sit_vpc" VPC
resource "aws_internet_gateway" "scaleup_sit_igw" {
    // Here we are attaching the IGW to the
    // scaleup_sit_vpc VPC
    vpc_id = aws_vpc.scaleup_sit_vpc.id

    // We are tagging the IGW with the name scaleup_sit_igw
    tags = {
        Name = "scaleup_sit_igw"
    }
}

// Create a group of public subnets based on the variable subnet_count.public
resource "aws_subnet" "scaleup_sit_public_subnet" {
    ...
    // Put the subnet into the "scaleup_sit_vpc" VPC
    vpc_id            = aws_vpc.scaleup_sit_vpc.id
    ...
    // We are tagging the subnet with a name of "scaleup_sit_public_subnet_" and
    // suffixed with the count
    tags = {
        Name = "scaleup_sit_public_subnet_${count.index}"
    }
}

// Create a group of private subnets based on the variable subnet_count.private
resource "aws_subnet" "scaleup_sit_private_subnet" {
    // count is the number of resources we want to create
    ...
    // We are tagging the subnet with a name of "scaleup_sit_private_subnet_" and
    // suffixed with the count
    tags = {
        Name = "scaleup_sit_private_subnet_${count.index}"
    }
}

// Create a public route table named "scaleup_sit_public_rt"
resource "aws_route_table" "scaleup_sit_public_rt" {
    // Put the route table in the "scaleup_sit_vpc" VPC
    vpc_id = aws_vpc.scaleup_sit_vpc.id
    ...
    // Gateway "scaleup_sit_igw"
    route {
        cidr_block = "0.0.0.0/0"
        gateway_id = aws_internet_gateway.scaleup_sit_igw.id
    }
}

// Here we are going to add the public subnets to the
// "scaleup_sit_public_rt" route table
resource "aws_route_table_association" "public" {
    ...
    // "scaleup_sit_public_rt" from above
    route_table_id = aws_route_table.scaleup_sit_public_rt.id

    // This is the subnet ID. Since the "scaleup_sit_public_subnet" is a
    // list of the public subnets, we need to use count to grab the
    // subnet element and then grab the id of that subnet
    subnet_id      = 	aws_subnet.scaleup_sit_public_subnet[count.index].id
}

// Create a private route table named "scaleup_sit_private_rt"
resource "aws_route_table" "scaleup_sit_private_rt" {
    // Put the route table in the "scaleup_sit_VPC" VPC
    vpc_id = aws_vpc.scaleup_sit_vpc.id
...
// route table "scaleup_sit_private_rt"
resource "aws_route_table_association" "private" {
    ...
    // Here we are making sure that the route table is
    // "scaleup_sit_private_rt" from above
    route_table_id = aws_route_table.scaleup_sit_private_rt.id

    // This is the subnet ID. Since the "scaleup_sit_private_subnet" is a
    // list of private subnets, we need to use count to grab the
    // subnet element and then grab the ID of that subnet
    subnet_id      = aws_subnet.scaleup_sit_private_subnet[count.index].id
}

// Create a security for the EC2 instances called "scaleup_sit_web_sg"
resource "aws_security_group" "scaleup_sit_web_sg" {
    // Basic details like the name and description of the SG
    name        = "scaleup_sit_web_sg"
    description = "Security group for scaleup_sit web servers"
...
resource "aws_db_subnet_group" "scaleup_sit_db_subnet_group" {
    ...
    name        = "scaleup_sit_db_subnet_group"
    description = "DB subnet group for scaleup_sit"

    // Since the db subnet group requires 2 or more subnets, we are going to
    // loop through our private subnets in "scaleup_sit_private_subnet" and
    // add them to this db subnet group
    subnet_ids  = [for subnet in aws_subnet.scaleup_sit_private_subnet : subnet.id]
}

// Create a DB instance called "scaleup_sit_database"
resource "aws_db_instance" "scaleup_sit_database" {
    ...
    // settings.database.db_name variable, which is set to "scaleup_sit"
...
// Create a key pair named "scaleup_sit_kp"
resource "aws_key_pair" "scaleup_sit_kp" {
    // Give the key pair a name
    key_name   = "scaleup_sit_kp"
    ...
    public_key = file("scaleup_sit_kp.pub")
}

// Create an EC2 instance named "scaleup_sit_web"
resource "aws_instance" "scaleup_sit_web" {
    ...
    // The subnet ID for the EC2 instance. Since "scaleup_sit_public_subnet" is a list
    ...
    // "scaleup_sit_public_subnet" and putting the EC2 instance in there
    subnet_id              = aws_subnet.scaleup_sit_public_subnet[count.index].id

    // The key pair to connect to the EC2 instance. We are using the "scaleup_sit_kp" key
    ...
    vpc_security_group_ids = [aws_security_group.scaleup_sit_web_sg.id]

    // We are tagging the EC2 instance with the name "scaleup_sit_db_" followed by
    // the count index
    tags = {
        Name = "scaleup_sit_web_${count.index}"
    }

    user_data = file("installDocker.sh")
}

// Create an Elastic IP named "scaleup_sit_web_eip" for each
// EC2 instance
resource "aws_eip" "scaleup_sit_web_eip" {
    ...
    // The EC2 instance. Since scaleup_sit_web is a list of
    ...
    instance = aws_instance.scaleup_sit_web[count.index].id
